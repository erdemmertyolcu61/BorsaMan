...
# BIST AI Trading Terminal — Codex Context

## Proje Amaci
BIST (Borsa Istanbul) hisselerini cok katmanli teknik + temel + akilli para + makro perspektifle
analiz eden, sinyal uretip risk yoneten, Codex Sonnet 4 tabanli yapay zeka yorumcusu ile destekli,
...
- **Backend**: `proxy/` — Vercel Serverless CORS proxy (10 domain whitelist + `/api/Codex`)
...
- **Model**: `Codex-sonnet-4-20250514`, `/api/Codex` proxy uzerinden, temperature 0.6
...
- **API dosyalari**: `src/utils/Codex.js` (setApiKey, buildExpertPrompt, askExpert, chatClaude,
...
- **Codex prompt**: `buildDailyPicksPrompt` her satira `HABER[kategori]=skor(count) "baslik"` ekler; system prompt kategori semantigini ve A/B/C notuna katkisini aciklar
...
- `graphify Codex install` → AGENTS.md'ye section + `.Codex/settings.json` PreToolUse Glob|Grep hook
- `graphify update .` → 177 file indexed → `graphify-out/{graph.json, graph.html, GRAPH_REPORT.md}`

### Hook'lar (`.Codex/hooks/`)
...
- [x] **Graphify Code-Knowledge-Graph (Codex Skill)** — proje genelinde token tasarrufu:
  - `graphifyy v0.5.0` (uv tool) — graph.json + GRAPH_REPORT.md + community wiki üretir
  - `graphify Codex install`: AGENTS.md'ye graphify section + `.Codex/settings.json` PreToolUse hook
  - **Hook'lar** (gist'ten install edildi, oyilmaztekin/e4bfb7d...):
    - `.Codex/hooks/block_search_tools.py` — PreToolUse:Bash: grep/find/rg/ag/fd algılarsa otomatik `graphify query` çalıştırır, sonuç varsa grep'i bloklayıp inject eder; yoksa fallback'e izin verir
    - `.Codex/hooks/subagent_graphify_context.py` — SubagentStart: subagent'lara graphify-first kuralını enjekte eder
...
- [x] Vercel proxy /api/Codex + 10 domain whitelist (KAP dahil)
...
- `renderSafeMarkdown(text)`: Codex AI yanitlari icin; escape → mini markdown (bold/italic/br) → DOMPurify tarama
...
- Not: `/api/Codex` Vercel handler'i `anthropic-beta` header'ini upstream'e pass etmeli
