// ════════════════════════════════════════════════════════════════════
// ForwardAccuracyPanel — honest next-day prediction accuracy display
// ════════════════════════════════════════════════════════════════════
//
// Surfaces the forward-test journal's ground-truth numbers. Deliberately
// leads with sample confidence so a tiny sample is never mistaken for proof.
// ════════════════════════════════════════════════════════════════════
    ...
    <div style={{ marginTop: 10 }}>
      ...
      {rows.map(([k, m]) => (
        <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '2px 0' }}>
          ...
          <span>
            <span style={{ color: m.accuracy >= 55 ? 'var(--green, #10b981)' : m.accuracy >= 45 ? 'var(--yellow, #eab308)' : 'var(--red, #ef4444)' }}>
              {pct(m.accuracy, 0)}
            </span>
            <span style={{ color: 'var(--t3, #6b7280)' }}> · {m.total} · </span>
            <span style={{ color: m.avgReturn >= 0 ? 'var(--green, #10b981)' : 'var(--red, #ef4444)' }}>
              {signed(m.avgReturn)}
            </span>
          </span>
        </div>
...
export default function ForwardAccuracyPanel({ journal }) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--t1, #e5e7eb)', letterSpacing: 0.5 }}>
            🎯 TAHMİN İSABETİ <span style={{ color: 'var(--cyan, #22d3ee)' }}>(Forward Test)</span>
          </div>
